...
class Dataset:
	def __init__(self, filenames, drop_cols, label_column, test=False):
		...
		'data/validation_data_stdevs.csv', 'data/training_data_entropy.csv', 'data/validation_data_entropy.csv', \
			'data/testing_data_means.csv', 'data/testing_data_entropy.csv',  'data/testing_data_stdev.csv']
	...
	def load_datasets(self):
		...
		for i,dataset in enumerate(self.files):
			...
			if 'btsv01' in dataset:
				df = df.drop([0], axis=0)
			if i==0:
				final_dataset = df
			elif i==1 :#or self.test:
				...
				left_merge_value = 'subject' if 'subject' in final_dataset.columns else 'subjectkey'
				right_merge_value = 'subject' if 'subject' in df.columns else 'subjectkey'
				final_dataset = pd.merge(final_dataset, df, left_on=left_merge_value, right_on=right_merge_value, how='outer')
	...
	def split_data_labels(self):
		if self.test: 
			self.final_dataset[self.label_column] = 0
		y = self.final_dataset[self.label_column].values.astype(float)
		self.subjects = self.final_dataset['subjectkey']
